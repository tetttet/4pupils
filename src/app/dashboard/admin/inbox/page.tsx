...
export default function Inbox() {
  ...
  const unreadCount = React.useMemo(
    () => mails.reduce((acc, m) => acc + (m.unread ? 1 : 0), 0),
    [mails],
  );
  ...
  const toggleStar = async (id: string) => {
    // optimistic
    setMails((prev) =>
      prev.map((m) => (m.mail_id === id ? { ...m, starred: !m.starred } : m)),
    );
    setActive((prev) => (prev ? { ...prev, starred: !prev.starred } : prev));

    // server
    const current = mails.find((m) => m.mail_id === id);
    const nextStar = !(current?.starred ?? active?.starred ?? false);

    await toast.promise(
      (async () => {
        try {
          await MailAPI.updateFlags(id, { starred: nextStar });
        } catch (e) {
          await loadList();
          if (activeId) setActive(await MailAPI.get(activeId));
          throw e;
        }
      })(),
      {
