...
export default function InboxView({
  ...
  const filtered = React.useMemo(() => {
    ...
    const only = mails.filter((m) => m.folder === folder);
    if (!q) return only;

    return only.filter((m) => {
  ...
  const unreadCount = React.useMemo(
    () =>
      mails.reduce(
        (acc, m) => acc + (m.folder === folder && m.unread ? 1 : 0),
        0,
      ),
    [mails, folder],
  );
  ...
  async function onConfirmAction() {
    ...
    try {
      ...
      else await archiveMail(confirmPayload.id);
  ...
  React.useEffect(() => {
    ...
    fetchUserById(active.sender_id || "")
      .then((user) => {
        if (isMounted) setSender(user);
      })
      .catch(() => {
        if (isMounted) setSender(null);
      });
  ...
  return (
    <div className="w-full bg-background">
      ...
      <motion.div
        ...
        animate={{ gridTemplateColumns: collapsed ? "110px 1fr" : "380px 1fr" }}
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-[#f8fafd]">
          <InboxHeader
            active={
              active
                ...
                : { id: "", subject: null, sender_id: null, starred: false }
            }
            name={name}
          ...
          <div className="mb-4 flex-1 overflow-auto rounded-3xl bg-white overflow-y-hidden">
            {!active ? (
              <InboxNoMessage />
            ) : (
